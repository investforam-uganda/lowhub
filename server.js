...
const PUBLIC_BACKEND_URL = (process.env.PUBLIC_BACKEND_URL || 'https://lowhub-1.onrender.com').replace(/\/+$/, '');
...
function extractStatus(body) {
  // MarzPay returns final status in transaction.status for both webhooks and
  // GET /transactions/{uuid}. Some responses also expose a top-level status
  // or event_type. Check all of them and explicitly handle cancellation.
  const candidates = [
    body?.event_type,
  ...
  if (candidates.some(v => ['completed','success','successful','succeeded'].includes(v)) || raw.includes('collection.completed')) return 'completed';
  if (candidates.some(v => ['failed','failure','declined','cancelled','canceled','error'].includes(v)) || /\b(fail|declin|cancel)\w*\b/.test(raw)) return 'failed';
  return 'pending';
}
...
async function pollMarzpayStatus(paymentDocId, txId) {
  // Mobile-money prompts can take a few minutes. The previous 25 x 6s loop
  // stopped after ~150s and could leave a perfectly valid payment stuck in
  // our Firestore as `processing`. Keep polling for up to 10 minutes and
  // use the documented transaction-details endpoint first, with the
  // collection-details endpoint as a compatibility fallback.
  const maxAttempts = 120;
  for (let i = 0; i < maxAttempts; i++) {
    if (i > 0) await new Promise(r => setTimeout(r, 5000));
    try {
      let statusData = null;
      let statusRes = await fetchWithTimeout(`${MARZPAY_BASE_URL}/transactions/${encodeURIComponent(txId)}`, {
        headers: { 'Authorization': marzpayAuthHeader(), 'Content-Type': 'application/json' }
      }, 15000);
      if (statusRes.ok) {
        statusData = await statusRes.json();
      } else {
        // Older MarzPay deployments expose collection details directly.
        statusRes = await fetchWithTimeout(`${MARZPAY_BASE_URL}/collect-money/${encodeURIComponent(txId)}`, {
          headers: { 'Authorization': marzpayAuthHeader(), 'Content-Type': 'application/json' }
        }, 15000);
      ...
      if (status === 'completed' || status === 'failed') {
        console.log(`[poll] transaction ${txId} reached final status: ${status}`);
        await applyPaymentStatus(txId, status, statusData);
        return;
      }
      if (i === 0 || i % 12 === 0) console.log(`[poll] transaction ${txId} still pending/processing (attempt ${i + 1}/${maxAttempts})`);
    } catch (e) {
      console.error(`[poll] attempt ${i + 1} error for ${txId}:`, e.message);
    }
  }
  console.warn('[poll] gave up waiting for transaction after 10 minutes', txId);
}
