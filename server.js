...
app.post('/api/admin/test-notification', async(req,res)=>{
  ...
  try{
    ...
    if(channel==='email') {
      ...
      const base=(c.baseUrl||'https://api.infobip.com').replace(/\/$/,''); r=await fetch(base+'/sms/2/text/advanced',{method:'POST',headers:{'Authorization':'App '+c.apiKey,'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({messages:[{destinations:[{to:normalizePhone(destination)}],from:c.sender,text:body}]})});
...
async function sendExternalNotification(userId, notificationId, n) {
  ...
  if(access.sms && pref.sms?.enabled!==false && phone && creds.sms){
    try{
      ...
      const r=await fetch(base+'/sms/2/text/advanced',{method:'POST',headers:{'Authorization':'App '+c.apiKey,'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({messages:[{destinations:[{to:normalizePhone(phone)}],from:c.sender,text:`${title}: ${body}`}]})});
    ...
    for (const doc of snap.docs) {
      ...
      try {
        if (p.forAll) {
          ...
          throw new Error('Push queue item has no userId or forAll flag.');
        }
        await doc.ref.update({ sent: true, sentAt: admin.firestore.FieldValue.serverTimestamp(), error: admin.firestore.FieldValue.delete() });
      } catch (sendErr) {
        console.error('[push] failed to send', doc.id, sendErr.message);
        // Keep failed jobs in the queue for a later retry instead of silently
        // marking them sent. This is especially important when Render starts
        // before Firebase credentials are ready or a device token is stale.
        const attempts = Number(p.attempts || 0) + 1;
        const permanent = /registration-token-not-registered|invalid-registration-token|Requested entity was not found/i.test(sendErr.message || '');
        if (permanent || attempts >= 5) {
          await doc.ref.update({ sent: true, error: sendErr.message, attempts, sentAt: admin.firestore.FieldValue.serverTimestamp() });
        } else {
          await doc.ref.update({ attempts, lastError: sendErr.message, lastAttemptAt: admin.firestore.FieldValue.serverTimestamp() });
        }
      }
    }
  } catch (e) {
...
async function sendToUser(userId, title, body, link) {
  ...
  if (!tokenDoc.exists) throw new Error(`No push token is registered for user ${userId}. Open LowHub on the user's device and enable notifications.`);
  const token = tokenDoc.data().token;
  if (!token) throw new Error(`Push token record for user ${userId} is empty.`);
  ...
  try {
    ...
    console.log('[push] sent to user', userId, messageId);
    return messageId;
  } catch (e) {
    // If FCM says the token is permanently invalid, remove it so the next
    // login/device can register a fresh token.
    if (/registration-token-not-registered|invalid-registration-token|Requested entity was not found/i.test(e.message || '')) {
